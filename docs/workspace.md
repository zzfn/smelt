...
|---|---|
| `Cmd+K` | 命令面板 |
| `Cmd+B` | 切换右侧 inspector 面板（FILES/GIT/TASKS/SKILL/SET） |
| `Esc` | 收起舞台上的全屏覆盖页（总览/文件树/Git/热力图/历史），回到当前会话 |
...
> 会话列表里是绿点）与**对话**（agent 经 ACP 接进 smelt 原生界面：结构化消息、
...
> 移除。设计稿原本的「64px 项目 rail」左右两列实测割裂（项目只剩一个字母、必须
> 先切项目才看得到会话），改为单列分组。
>
